...
template<class T>
void check(const char* text, T result)
{
  try
  {
    Calculator<T> calc(text);
    T res = calc.expression();
    if (fabs( res - result) > 0.00001)
    {
        cout << "Wrong result at expression: "  << text << endl;
        throw "Wrong test answer";
    }
  }
  catch(const char* str)
  {
    cout << str << endl;
    throw "Error: repair algorithm!";
  }
}
...
int main(int argc, char* argv[])
{
  ...
  try
  {
    ...
    check<int>("(2*2  )", 4);
    check<int>("(((2 + 2)) * 2)", 8);
    check<int>("(5 + 3 * (9 / Pi) * 4) + (1 * e)", 43);

    check<double>("((2.0 + Pi) - 3.0) * (4.0 - -2.0)", 12.84);
    check<double>("Pi + e", 5.84);
    check<double>("(5 + 3 * (9 / Pi) * 4)", 39.3949);
    check<double>("(1.3+ - 1.2)", 0.1);

    check<long>("2 + 3 * 4 - -2", 16);

...
public:
    ...
    public:
        ...
        ~column(){  //default destructor
//            delete[] t;                         !!! ASK WHY THIS ID NOT WORK !!!
//            std :: cout << "Del column\n";
        }
        T& operator[](int i){
            assert(i>=0 and i<size_c);
            return t[i];
        }
...
//        a = new column[0];
    ...
    ~Matrix(){
//        std :: cout << "Del rows\n";            !!! ASK WHY THIS ID NOT WORK !!!
//        delete[] a;
    }
    
    Matrix(const Matrix& new_A){ // copy-constructor
        N = new_A.N;
        M = new_A.M;
        a = new_A.a;
    }
    ...
    Matrix& operator=(const Matrix& new_A){ // copy
        if (this == &new_A)
            return *this;
//        this->Clear();
        N = new_A.N;
    ...
    Matrix(Matrix&& new_A) { // move-constructor
        N  = new_A.N;
        M  = new_A.M;
        a = new_A.a;
    }
    ...
    Matrix& operator=(Matrix&& new_A) { // move
        if (this == &new_A) {
            return *this;
        }
        //data_column.clear();
...
template <typename T> class run_test{
private:
    bool currentTest;
    bool allPassed;
public:

    ...
    static std::pair<int, int> parse(std::string &s, int start_pos) {
        ...
        while (start_pos < s.size() && '0' <= s[start_pos] && s[start_pos] <= '9') {
            ...
            if (result * sign > MyTypeInfo<int>::max) {
                throwError("Error while parsing int type, the input is too big!\n");
            } else if (result * sign < MyTypeInfo<int>::min) {
                throwError("Error while parsing int type, the input is too small!\n");
            }
            start_pos++;
        }
    ...
    static std::pair<int, long> parse(std::string &s, int start_pos) {
        ...
        while (start_pos < s.size() && '0' <= s[start_pos] && s[start_pos] <= '9') {
            ...
            if (result * sign > MyTypeInfo<long>::max) {
                throwError("Error while parsing long type, the input is too big!\n");
            } else if (result * sign < MyTypeInfo<long>::min) {
                throwError("Error while parsing long type, the input is too small!\n");
            }
            start_pos++;
        }
        return std::make_pair(start_pos, result * sign);
...
// Double parse
class DoubleParser : ErrorThrower {
public:
    // Return the next position in string, and the result of parsing
    static std::pair<int, double> parse(std::string &s, int start_pos) {
        ...
        while (start_pos < s.size() && (('0' <= s[start_pos] && s[start_pos] <= '9') || s[start_pos] == '.')) {
            if (s[start_pos] == '.') {
                if (was_dot) {
                    throwError("Wrong number written by double type\n");
                } else {
                    was_dot = true;
                }
            } else {
                ...
                if (result * sign > MyTypeInfo<double>::max) {
                    throwError("Error while parsing double type, the input is too big!\n");
                } else if (result * sign < MyTypeInfo<double>::min) {
                    throwError("Error while parsing double type, the input is too small!\n");
                }
            }
            start_pos++;
        }
...
public:
    ...
    static std::pair<int, T> parse(std::string &s, int start_pos) {
        if (IsSame<T, int>::result) {
            ...
            throwError("Unknown type in Parser\n");
        }
    }
};

template <class T>
class Expression : ErrorThrower {
public:
    // Empty constructor
    Expression() {}
...
private:
        ...
        while (cur_pos < len) {
            if (expr[cur_pos] == '*' || expr[cur_pos] == '/') {
                ...
                if (isMult) value *= cur_value;
                    ...
                    if (cur_value == 0) throwError("Zero division!");
                    value /= cur_value;
                }
    ...
    // get constant
    T constValue() {
        if (!IsSame<T, double>::result) throwError("Try to use constant in no-double type");
        double sign = 1;
        if (expr[cur_pos] == '-') {
            sign = -1;
            cur_pos++;

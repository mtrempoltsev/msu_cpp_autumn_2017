...
T getNumber(const char *start, const char *end) {
    ...
    while (start < end) {
        if (((max - (*start - 48)) / 10) < result)
            throw std::out_of_range("Given number too big for specified type");
    ...
    switch (token) {
        case Token::BinaryMinus: {
            if ((rValue > 0) && (min + rValue > lValue))
                ...
                out_of_range = true;
            if (out_of_range)
                throw std::out_of_range("Number went out of bounds during calculation");
        ...
        default:
            throw std::logic_error("Could not solve tree");
    }
}
...
int use_calculator(char const *text) {
    ...
    if (not isvalid) {
        ...
        try{
            ...
            cout << "Exception: overflow\n" << e.what() << endl;
            return 1;
        } catch (const std::logic_error& e){
            cout << "Exception: logic error\n" << e.what() << endl;
            return 1;
        } catch (...){

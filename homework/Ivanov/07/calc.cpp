    ...
    static pair<int, int> parse(string &s, size_t start_pos) {
        ...
        while (start_pos < s.size() && '0' <= s[start_pos] && s[start_pos] <= '9') {
            ...
            if (result * sign > MyTypeInfo<int>::max) {
                throw ("Error while parsing int type, the input is too big!");
            } else if (result * sign < MyTypeInfo<int>::min) {
                throw ("Error while parsing int type, the input is too small!");
            }
            start_pos++;
        }
    ...
    static std::pair<int, long> parse(std::string &s, size_t start_pos) {
        ...
        while (start_pos < s.size() && '0' <= s[start_pos] && s[start_pos] <= '9') {
            ...
            if (result * sign > MyTypeInfo<long>::max) {
                throw ("Error while parsing long type, the input is too big!");
            } else if (result * sign < MyTypeInfo<long>::min) {
                throw ("Error while parsing long type, the input is too small!");
            }
            start_pos++;
        }
        return std::make_pair(start_pos, result * sign);
...
public:
    static std::pair<int, double> parse(std::string &s, size_t start_pos) {
        ...
        while (start_pos < s.size() && (('0' <= s[start_pos] && s[start_pos] <= '9') || s[start_pos] == '.')) {
            if (s[start_pos] == '.') {
                if (was_dot) {
                    throw ("Wrong number written by double type");
                } else {
                    was_dot = true;
                }
            } else {
                ...
                if (result * sign > MyTypeInfo<double>::max) {
                    throw ("Error while parsing double type, the input is too big!");
                } else if (result * sign < MyTypeInfo<double>::min) {
                    throw ("Error while parsing double type, the input is too small!");
                }
            }
            start_pos++;
        }
    ...
    T value() {
        ...
        while(pos < len) {
            if(str[pos] == '-' || str[pos] == '+') {
                ...
                throw (unknownSymbol(str[pos], "sign or bracket"));
            }
        }
        if(str[pos] != ')') {
            throw (unknownSymbol(str[pos], ")"));

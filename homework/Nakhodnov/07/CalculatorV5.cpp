...
//  ласс дл€ хранени€ лексем разбираемой строки
template <class T>
class Token {
public:
    enum TokenTypes {
...
//  ласс, разбивабщий строку на лексемы
template <class T>
class Tokenizer {
public:
    Tokenizer(char *s, unordered_map<string, T> mp) : S(s), constants(mp) {}
    // ѕолучаем первый токен из строки S
    Token<T> getToken() {
        while (*S && isspace(*S)) {
            ++S;
        }
...
private:
    ...
    int balance = 0;
    /*
    ¬ыдел€ем из строки число <number>:
...
private:
    ...
    /*
    ¬ычисл€ем значение лексической единицы <prim>:
    <prim> ::= <number>|-<number>
    */
    T getPrim() {
    ...
    /*
    ¬ычисл€ем значение лексической единицы <term>:
    <term> ::= <prim>|<term> * <prim>|<term> / <prim>
    “.е. выполн€ем обработку операций делени€ и вычитани€
    */
    T getTerm() {
        T left, tmp;
        Token<T> token;
    ...
    /*
    ¬ычисл€ем значение лексической единицы <expr>:
    <expr> ::= <term>|<expr> + <term>|<expr> - <term>|'('<expr>')'
    “.е. выполн€ем обработку операций сложени€ и вычинани€
    */
    T getExpr() {
        Token<T> token;
        T left = getTerm();
